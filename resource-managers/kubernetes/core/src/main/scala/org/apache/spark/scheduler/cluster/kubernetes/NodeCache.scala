...
private[spark] class NodeCache(
    val client: KubernetesClient,
    val dsNamespace: String,
    val dsLabels: Map[String, String]) extends Logging {

  // TODO: update the cache as needed using a watch.
  // TODO: check for multiple shuffle services on the same pod.
  ...
  def start(): Unit = {
    scheduler = ThreadUtils.newDaemonSingleThreadExecutor("kubernetes-node-cache")
  ...
  private def addShufflePodToCache(pod: Pod): Unit = {
    shufflePodCache.get(pod.getSpec.getNodeName) match {
      case Some(registeredPodName) =>
